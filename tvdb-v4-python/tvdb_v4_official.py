...
class Request:
    ...
    def make_request(self, url, if_modified_since=None, data=None):
        if data:
            data = parse.urlencode(data).encode()
            req = urllib.request.Request(url, data=data, method='POST') #FIXME!
            req.add_header('Content-Type', 'application/json')
        else:
...
class TVDB:
    ...
    def add_to_user_favorites(self, data=None) -> dict:
        """POST adds to user favorites"""
        url = self.url.construct('user/favorites/foo')
        return self.request.make_request(url, data=data)